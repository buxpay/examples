...
async def donate(interaction: discord.Interaction, amount_of_robux: int):
    res = requests.post("http://buxpay.xyz/payments/create",json={"api_key":api_key,"price":int(amount_of_robux)}).json() # create an invoice
    ...
    def check_if_bought():
      check = requests.get(f"http://buxpay.xyz/payments/info",json={"api_key":api_key,"uid":uid})
